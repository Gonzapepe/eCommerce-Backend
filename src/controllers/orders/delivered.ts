...
export const delivered = async (
  ...
  try {
    ...
    const getOrders = await response
      ...
      .where("order.delivered_date IS NOT NULL")
      .skip((page - 1) * perPage)
      .take(perPage)
      .getMany();
    total = await response.where("order.delivered_date IS NOT NULL").getCount();
    ...
    res.customSuccess(200, "Lista de ordenes despachadas: ", {
      orders,
      total,
      page,
